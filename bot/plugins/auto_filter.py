...
async def connect(bot: Bot, update):
    ...
    if responce:
        ...
        await bot.send_message(
            chat_id=group_id,
            text=f"Having Problem While Connecting...Report @NickxFury",
            reply_to_message_id=update.message_id
        )
        return
...
async def disconnect(bot, update):
    ...
    if responce:
        ...
        await bot.send_message(
            chat_id=group_id,
            text=f"Having Problem While Disconnecting...Report @NickxFury",
            reply_to_message_id=update.message_id
        )
        return
...
async def auto_filter (bot, update):
    ...
    if len(results) == 0:
        ...
        await bot.send_message(
            chat_id = update.chat.id,
            text=f"Found {(len(results))} [👉 Join Channel Before Accesing These Buttons](https://t.me/joinchat/UKGLK6xq0Rg3YjQ1) Results For Query: <code>{query}</code>",
...
async def cb_handler(bot, query:CallbackQuery, group=1):
    ...
    if cb_data == "start":
        buttons = [[
            InlineKeyboardButton('My Dev 👨‍🔬', url='https://t.me/NickxFury'),
